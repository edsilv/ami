...
uniform sampler3D uIntensityData;
uniform sampler3D uGradientData;
...
void main(void) {
  ...
  for(int i = 0; i < MAX_STEPS; i++){
    // Prevent comparison with non-constant error in IE & Edge
    if (i >= uSteps) {
      break;
    }
    ...
    gradient = texture(uGradientData, currentVoxel).xyz;
