...
export default class FrameModel extends BaseModel {
                 ...
                 public spacingXY() {
                   ...
                   if (this.pixelSpacing) {
                     ...
                     spacingXY[0] = 1.0;
                     spacingXY[1] =
                       (1.0 * this.pixelAspectRatio[1]) / this.pixelAspectRatio[0];
                   }
                  ...
                  * @return {*}
                  */
                 public getPixelData(column: any, row: any) {
                   if (
                     column >= 0 &&
                  ...
                  * @return {*}
                  */
                 public setPixelData(column: any, row: any, value: any) {
                 ...
                 public _frameToCanvas() {
                   ...
                   if (this._windowWidth && this._windowCenter !== null) {
                     ...
                     params.min = Math.max(
                       (this._windowCenter - this._windowWidth / 2 - intercept) /
                         slope,
                       params.min
                     );
                     params.max = Math.min(
                   ...
                   if (this._numberOfChannels === 1) {
                     for (let i = 0; i < dimension; i++) {
                       const normalized = this._pixelTo8Bit(
                         this._pixelData[i],
                         params
                       );
                       data[4 * i] = normalized;
                     ...
                     for (let i = 0; i < dimension; i++) {
                       data[4 * i] = this._pixelTo8Bit(
                         this._pixelData[3 * i],
                         params
                       );
                       data[4 * i + 1] = this._pixelTo8Bit(
                         this._pixelData[3 * i + 1],
                         params
                       );
                       data[4 * i + 2] = this._pixelTo8Bit(
                         this._pixelData[3 * i + 2],
                         params
                       );
                       data[4 * i + 3] = 255; // alpha channel (fully opaque)
                     }
                  ...
                  * @return {Number}
                  */
                 public _pixelTo8Bit(value: any, params: any) {
                   // values equal to pixelPaddingValue are outside of the image and should be ignored
                   let packedValue =
                     value <= params.min || value === params.padding ? 0 : 255;

                   if (value > params.min && value < params.max) {
                     packedValue = Math.round(
                       ((value - params.min) * 255) / params.range
                     );
                   }

                   return Number.isNaN(packedValue)
                     ? 0
                     : params.invert
                     ? 255 - packedValue
                     : packedValue;
                  ...
                  */
                 public _compareArrays(reference: any, target: any) {
                   // could both be null
                   if (reference === target) {
                     return true;
                   }
                 ...
                 set sliceThickness(sliceThickness: any) {
                 ...
                 set imagePosition(imagePosition: any) {
                 ...
                 set imageOrientation(imageOrientation: any) {
                 ...
                 set pixelSpacing(pixelSpacing: any) {
                 ...
                 set pixelAspectRatio(pixelAspectRatio: any) {
                 ...
                 set dimensionIndexValues(dimensionIndexValues: any) {
                 ...
                 set instanceNumber(instanceNumber: any) {
                 ...
                 set sopInstanceUID(sopInstanceUID: any) {
                 ...
                 set pixelPaddingValue(pixelPaddingValue: any) {
                 ...
                 set referencedSegmentNumber(referencedSegmentNumber: any) {
